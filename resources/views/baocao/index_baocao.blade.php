        ...
        <div class="row" >
            ...
            <div class="col-md-3">
                ...
                <select id="ky" name="ky" class="form-control">
                    @foreach($ky as $k)
                        @if($ky[0]->nam == DateTime::createFromFormat('Y-m-d H:i:s',$k->tungay)->format('Y'))
                        <option value="{{route('xembaocao_ky',$k->id)}}">
                            Kỳ {{$k->tuan}}: Từ {{DateTime::createFromFormat('Y-m-d H:i:s',$k->tungay)->format('d/m/Y')}} đến {{DateTime::createFromFormat('Y-m-d H:i:s',$k->denngay)->format('d/m/Y')}}
                        </option>
                        @endif
                    @endforeach
                </select>
